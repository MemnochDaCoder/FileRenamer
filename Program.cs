...
class Program
{
    static async Task Main(string[] args)
    {
        while (true)
        {
            ...
            {
                Console.WriteLine($"{i + 1}. {proposedChanges[i].ProposedFileName}");
            }
            ...
            if (success)
            {
                Console.WriteLine("Renaming process completed successfully. Do you want to go again? (yes/no)");
                string? userChoice = Console.ReadLine()?.ToLower();

                if (userChoice == "no" || userChoice == "exit" || userChoice == null)
                {
                    break;
                }
                else
